...
def solve_poisson_with_conjugate_gradient(rho, phi, world, tol=1e-8, max_iter=5000):
    ...
    so the equation is solvable on a periodic domain. The solution is normalized to
    zero mean to fix the constant mode.
    ...
    def body_fun(state):
        ...
        return normalize(phi_next), r_next, p_next, k + 1
