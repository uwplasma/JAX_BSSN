...
axes[0].plot(r_ref * 1e6, alpha_dev_newton, 'k--', lw=2,
             label=r'Newtonian $\Phi(r)$')
axes[0].plot(r_centers * 1e6, bssn_dev, 'o-', ms=4, lw=1.2,
             label=f'BSSN $\\alpha - 1$ (t = {float(times_mass[-1]):.2e} Mm)')
axes[0].axvline(float(R_sphere) * 1e6, ls=':', color='grey', label=f'R = {float(R_sphere):.2e} Mm')
...
axes[1].plot(r_ref * 1e6, alpha_dev_newton, 'k--', lw=2, label=r'Newtonian $\Phi(r)$')
axes[1].plot(r_centers * 1e6, bssn_dev, 'o-', ms=4, lw=1.2, color='tab:red',
             label=f'BSSN $\\alpha - 1$')
axes[1].axvline(float(R_sphere) * 1e6, ls=':', color='grey')
...
plt.savefig("planetary_mass_plot_3d.png", dpi=300)
print("Plot saved as planetary_mass_plot_3d.png")
