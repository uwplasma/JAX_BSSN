...
def build_radially_perturbed_initial_data(coords, amplitude=1e-3, sigma=1.0, dtype=jnp.float32):
    ...
    # Smooth Gaussian radial perturbation pulse
    pulse = amplitude * jnp.exp(-(r3d**2) / (sigma**2))

    # Conformal factor chi = e^(-4 phi) = 1 / (1 + pulse)
    conformal_factor = 1.0 / (1.0 + pulse)

    # Conformal spatial metric bar_gamma_ij = delta_ij
    ...
    # Standard flat/vacuum initial BSSN fields
...
def run_perturbed_bssn_simulation(
    ...
    # Force asynchronous JAX work to finish here so OOM errors surface at the
    # actual simulation that caused them rather than at a later float() call.
...
def main():
    ...
    # Compute finest mesh reference solution
    print(f"\n[1/2] Computing finest reference solution on Nx = {nx_fine}...")
    try:
        dx_fine, fine_vars = run_perturbed_bssn_simulation(
            nx_fine,
            domain_size,
            dt_factor,
            target_time,
    ...
    # TARGET: x-component of the shift vector
    fine_shift_x = fine_vars.shift[0]

    dx_list = []
    l2_errors = []

    # Run coarse grid simulations and compute L2 error norm against downsampled fine solution
    ...
    # Reference scaling lines (2nd & 4th order)
